...
# perform correlation analyses between gene with highest absolute expression in GPR and flux for each reaction
# rxn.top.genes <- c()
# for (rxn in rownames(FBA_mat_incl1)){
#   get.flux <- abs(FBA_mat_incl1[match(rxn,rownames(FBA_mat_incl1)),c(1:51,80:89)])
#   get.all.genes <- unique(as.vector(strsplit2(ihuman.geneName$GeneAssociationName[match(rxn,ihuman.geneName$rxnRetired)],split="\\(|\\)| or | and ")))
#   if (any(grep("ENSG",get.all.genes))){
#     get.all.genes <- get.all.genes[-grep("ENSG",get.all.genes)]
#   }
#   if (any(get.all.genes%in%c("","0"))){
#     get.all.genes <- get.all.genes[-which(get.all.genes%in%c("","0"))]
#   }
#   if (length(get.all.genes)==0){
#     next
#   }
#   if (length(get.all.genes)==1 && is.na(get.all.genes)){
#     next
#   }
#   if (length(get.all.genes)==1){
#     get.geneExp <- metgenes_patient_geneNames[match(get.all.genes,rownames(metgenes_patient_geneNames)),c(1:51,80:89)]
#     get.cor <- cor.test(get.flux,get.geneExp,method="spearman")
#     rxn.top.genes <- rbind(rxn.top.genes,c(rxn,get.all.genes,get.cor$estimate[["rho"]]))
#   } else {
#     row.meds <- rowMedians(metgenes_patient_geneNames[na.omit(match(get.all.genes,rownames(metgenes_patient_geneNames))),c(1:51,80:89)])
#     if (all(row.meds!=0)){
#       get.max.gene <- get.all.genes[which(row.meds==max(row.meds))]
#       get.geneExp <- metgenes_patient_geneNames[match(get.max.gene,rownames(metgenes_patient_geneNames)),c(1:51,80:89)]
#       get.cor <- cor.test(get.flux,get.geneExp,method="spearman")
#       rxn.top.genes <- rbind(rxn.top.genes,c(rxn,get.max.gene,get.cor$estimate[["rho"]]))
#     }
#   }
# }
# 
# rxn.top.genes <- cbind(rxn.top.genes,ihuman.geneName[match(rxn.top.genes[,1],ihuman.geneName$rxnRetired),3:6])
# rxn.top.genes <- as.data.frame(rxn.top.genes)
# colnames(rxn.top.genes)[1:3] <- c("Rxn","Gene","Spearman.Rho")
# rxn.top.genes$Spearman.Rho <- as.numeric(rxn.top.genes$Spearman.Rho)
# 
# # Top positively correlated subsystems
# rxn.top.genes.pos <- rxn.top.genes[which(rxn.top.genes$Spearman.Rho>0.5),]
# rxn.top.genes.pos.table <- as.data.frame(table(rxn.top.genes.pos$SUBSYSTEM))
# rxn.top.genes.pos.table$total <- FBA_mat_nonzero_subsys.table$Freq[match(rxn.top.genes.pos.table$Var1,FBA_mat_nonzero_subsys.table$Var1)]
# rxn.top.genes.pos.table$Pct <- (rxn.top.genes.pos.table$Freq / rxn.top.genes.pos.table$total) * 100
# rxn.top.genes.pos.table <- rxn.top.genes.pos.table[which(rxn.top.genes.pos.table$Freq>5),]
# rxn.top.genes.pos.table <- rxn.top.genes.pos.table[order(rxn.top.genes.pos.table$Pct, decreasing = T),]
# 
# 
# library(scales)
# ggplot(rxn.top.genes.pos.table[1:10,], aes(x = reorder(Var1, Pct), y = Pct)) +
#   geom_point(aes(size = Pct, color = Freq)) +
#   scale_size_continuous(name = "Percentage",range = c(4,8)) +
#   scale_color_gradientn(colors = c("blue", "red"),
#                         values = rescale(c(5, 45)),
#                         name = "Frequency") +
#   labs(title = "Top 10 Pos (> 0.5) Gene Expression/Flux Correlations", x = "Subsystem", y = "Percentage") + ylim(c(30,50))+
#   coord_flip() + 
#   theme_minimal() + 
...
# 
# 
# # Top negatively correlated subsystems
# rxn.top.genes.neg <- rxn.top.genes[which(rxn.top.genes$Spearman.Rho< -0.5),]
# rxn.top.genes.neg.table <- as.data.frame(table(rxn.top.genes.neg$SUBSYSTEM))
# rxn.top.genes.neg.table$total <- FBA_mat_nonzero_subsys.table$Freq[match(rxn.top.genes.neg.table$Var1,FBA_mat_nonzero_subsys.table$Var1)]
# rxn.top.genes.neg.table$Pct <- (rxn.top.genes.neg.table$Freq / rxn.top.genes.neg.table$total) * 100
# rxn.top.genes.neg.table <- rxn.top.genes.neg.table[which(rxn.top.genes.neg.table$Freq>5),]
# rxn.top.genes.neg.table <- rxn.top.genes.neg.table[order(rxn.top.genes.neg.table$Pct, decreasing = T),]
# 
# ggplot(rxn.top.genes.neg.table, aes(x = reorder(Var1, Pct), y = Pct)) +
#   geom_point(aes(size = Pct, color = Freq)) +
#   scale_size_continuous(name = "Percentage",range = c(4,8)) +
#   scale_color_gradientn(colors = c("blue", "red"),
#                         values = rescale(c(10, 30)),
#                         name = "Frequency") +
#   labs(title = "Top 10 Pos (< -0.5) Gene Expression/Flux Correlations", x = "Variables", y = "Percentage") +
#   coord_flip() + 
#   theme_minimal() + 
#   theme(plot.title = element_text(hjust = 0.5),
#         panel.background = element_rect(fill = "white", color = NA),  # White background
...
## read in net enzymatic level (NEL) calculations from GPR
GPR <- read.table("GPR.txt", sep = '\t')
GPR_FBA_incl_1 <- GPR[match(rownames(FBA_mat_incl1),rownames(GPR)),]

# ## correlation analysis between gene in GPR and metabolic flux for each reaction
# all.geneRule.flux.corrs <- c()
# for (i in rownames(FBA_mat_incl1)){#rownames(FBA_mat)){
#   findrxn <- abs(as.numeric(FBA_mat[match(i,rownames(FBA_mat)),c(1:51,80:89)]))
#   get.geneRule<- ihuman$`GENE ASSOCIATION`[match(i,ihuman$rxnRetired)]
#   get.geneRule <- strsplit2(get.geneRule,split=' or | and |\\(|\\)')
#   for (j in get.geneRule){
#     if (nchar(j)==15 & j%in%rownames(metgenes_patient)){
#       findgene <- as.numeric(metgenes_patient[match(j,rownames(metgenes_patient)),c(1:51,80:89)])
#       get.cor <- cor.test(findrxn,findgene, method = 'spearman')
#       if (!(is.na(get.cor$estimate[['rho']]))){
#         all.geneRule.flux.corrs <- rbind(all.geneRule.flux.corrs,c(i,j,metgenes$V2[match(j,metgenes$V1)],get.cor$estimate[['rho']]))
#       }
#     }
#   }
# }
# 
# 
# ## correlation analysis between NEL from GPR and metabolic flux for each each reaction
# all.GPR.flux.corrs <- c()
# for (i in rownames(FBA_mat_incl1)){#rownames(FBA_mat)){
#   findrxn <- abs(as.numeric(FBA_mat[match(i,rownames(FBA_mat)),c(1:51,80:89)]))
#   findGPR<- as.numeric(GPR[match(i,rownames(GPR)),c(1:51,80:89)])
#   get.cor <- cor.test(findrxn,findGPR, method = 'spearman')
#   if (!(is.na(get.cor$estimate[['rho']])) ){
#     all.GPR.flux.corrs <- rbind(all.GPR.flux.corrs,c(i,get.cor$estimate[['rho']]))
#     }
# }
# 
# ## correlation analysis between NEL from GPR and gene in GPR for each each reaction
# all.GPR.gene.corrs <- c()
# for (i in rownames(FBA_mat_incl1)){#rownames(FBA_mat)){
#   findGPR<- as.numeric(GPR[match(i,rownames(GPR)),c(1:51,80:89)])
#   get.geneRule<- ihuman$`GENE ASSOCIATION`[match(i,ihuman$rxnRetired)]
#   get.geneRule <- strsplit2(get.geneRule,split=' or | and |\\(|\\)')
...
#     if (nchar(j)==15 & j%in%rownames(metgenes_patient)){
#       findgene <- as.numeric(metgenes_patient[match(j,rownames(metgenes_patient)),c(1:51,80:89)])
#       get.cor <- cor.test(findGPR,findgene, method = 'spearman')
#       if (!(is.na(get.cor$estimate[['rho']]))){
#         all.GPR.gene.corrs <- rbind(all.GPR.gene.corrs,c(i,j,metgenes$V2[match(j,metgenes$V1)],get.cor$estimate[['rho']]))
#       }
#     }
#   }
# }
